...
% fminsearch -------------------------------------------------------------------------------------------
fun_fminsearch = @(x) 20*(-x(1)^2 + 2*x(2))^2 + (x(1) - 1)^2 + (x(2) - 2)^2;
...
[x_opt_fminsearch, fval_fminsearch, exitflag_fminsearch, output_fminsearch] = ...
    fminsearch(fun_fminsearch, X0, options_fminsearch);
...
% use fminunc: 
[x_opt_fminunc, fval_fminunc, exitflag_fminunc, output_fminunc, grad_fminunc, hessian_fminunc] = ...
    fminunc( @optim_prob2, [-1,2], options_fminunc);
...
lb     = [-3, -3];
ub     = [+3, +3];
options_fmincon = optimset('Disp','iter', 'PlotFcns', {@optimplotfval, @optimplotx});

[x_opt_fmincon, fval_fmincon, exitflag_fmincon, output_fmincon] = ...
    fmincon(@objective3, X0, [], [], [], [], [], [], @mycon_c_ceq, options_fmincon);
...
for ii = 1:10
    ...
    X0 = [ 6*rand(1) - 3; 6*rand(1) - 3 ];
    
    [x_opt_fmincon2{ii}, fval_fmincon2{ii}, exitflag_fmincon2{ii}, output_fmincon2{ii}] = ...
    fmincon(@objective3, X0, [], [], [], [], [], [], @mycon_c_ceq, options_fmincon);

    x_opt_fmincon2_all(ii,:) = x_opt_fmincon2{ii}';
...
u_chirp = chirp(t, f_start, t(end), f_end,'linear', -90);
figure;
subplot(2,1,1);
...
% lsim(sys,u,t,x0) ;
y_lsim = lsim(G, u_chirp2, t);
y_noise = y_lsim + 0*0.010*randn(size(y_lsim)); %*****************************************
...
[Ujw, f_vec] = cal_FFT(u_chirp2, fs); % Ujw = FFT of u_chirp2
% [Yjw, f_vec] = cal_FFT(y_lsim,   fs); % Yjw = FFT of y_lsim
[Yjw, f_vec] = cal_FFT(y_noise,   fs); % Yjw = FFT of y_lsim

figure,plot(f_vec, abs(Ujw));
figure,plot(f_vec, abs(Yjw));

Hjw = Yjw./Ujw; % --> FFT of Transfer Function 
Mag_Hjw = 20*log10( abs(Hjw) );  % --> [db]
Phs_Hjw = (angle(Hjw))*180/(pi); % --> [deg]
...
data_idfrd = idfrd(Hjw, 2*pi*f_vec, Ts);
nx = 4;

ssest_option = ssestOptions('Focus', 'simulation', ...
                            'EnforceStability', true, ...
                            'Display', 'on');
ss_4 = ssest(data_idfrd, nx, 'Form', 'modal', ssest_option);

ss_est4 = ss( ss_4.a, ss_4.b, ss_4.c, ss_4.d); % *********************
...
[mag_ss_est4, phs_ss_est4] = bode(ss_est4, 2*pi*f_vec);
mag_ss_est4 = 20*log10(squeeze(mag_ss_est4));
phs_ss_est4 = squeeze(phs_ss_est4);

% True Model G
[mag_ss_G, phs_ss_G] = bode(G, 2*pi*f_vec);
mag_ss_G = 20*log10(squeeze(mag_ss_G));
phs_ss_G = squeeze(phs_ss_G);
